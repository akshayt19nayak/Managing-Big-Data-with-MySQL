...
#Problem11
SELECT TOP 5 final.store,final.city,final.state,final.deptdesc,SUM(final.November)/SUM(final.NovDays)AS nadr,SUM(final.December)/SUM(final.DecDays) AS dadr,(dadr-nadr)/nadr
...
FROM(SELECT my.sku, my.store,my.month_num,my.year_num
FROM (SELECT sku, amt, store, saledate, EXTRACT(MONTH from saledate) AS month_num, EXTRACT(YEAR from saledate) AS year_num
FROM trnsact
WHERE stype='P') AS my JOIN (SELECT skst.sku,smsa.state,smsa.city,smsa.store,skd.deptdesc
...
ON my.sku=csd.sku AND my.store=csd.store
WHERE month_num=11 OR month_num=12
GROUP BY my.store,my.month_num,my.year_num
...
GROUP BY final.store,final.city,final.state,final.deptdesc
